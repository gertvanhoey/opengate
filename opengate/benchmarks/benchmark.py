...
def _build_and_run(num_threads: int, activity_bq: float, duration_s: float) -> float:
    ...
    # Pile-up actor.
    if PILE_UP:
        pu = sim.add_actor("DigitizerPileupActor", "pile-up")
        pu.input_digi_collection = previous_actor.name
        pu.group_volume = "pet_crystal"
        pu.authorize_repeated_volumes = True
        pu.time_window = 0.5 * ns
        pu.output_filename = ""
        previous_actor = pu

    # Dead time actor.
    if DEAD_TIME:
        dt = sim.add_actor("DigitizerDeadTimeActor", "dead time")
        dt.input_digi_collection = previous_actor.name
        dt.group_volume = "pet_crystal"
    ...
    if COINCIDENCE:
        ...
        cc.input_digi_collection = previous_actor.name
        cc.window = 3 * ns
